...
export interface ShareCrudInternalOptions {
  type: string;
  apiBasePath: string;
...
export interface ShareCrudInternalReturn {
  shares: Share[];
  isLoading: boolean;
  isCreating: boolean;
  error: string | null;
...
export function useShareCrudInternal({
  type,
  apiBasePath,
  visibleFields,
  params,
}: ShareCrudInternalOptions): ShareCrudInternalReturn {
  ...
  const createShare = useCallback(
    async (expiresAt?: Date): Promise<{ token: string; url: string } | null> => {
      setIsCreating(true);
      setError(null);
      try {
